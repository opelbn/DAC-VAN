...
class MCPClient:
    def __init__(self):
        ...
        # Remove _load_siem_config, use config directly
        ...
        self.siem_handler = SIEMHandler(self.config, self.debug)
        if self.config.pipeline_context.get("ci_cd") == "GitHub Actions runner":
            logging.info("Running in CI/CD environment: %s", self.config.pipeline_context["runner_details"])
        logging.info("MCPClient initialized with debug=%s, SIEM=%s", self.debug, self.config.siem_platform)
    ...
    def run_pipeline(self, rule_path, args):
        ...
        try:
            ...
            if args.parse_only:
                ...
                timings['parsing'] = time.perf_counter() - parsing_start
                timings['total'] = time.perf_counter() - total_start
                logging.info(f"Timings: { {k: f'{v:.3f}s' for k, v in timings.items()} }")
                return validity
            else:
            ...
            logging.info(f"Timings: { {k: f'{v:.3f}s' for k, v in timings.items()} }")
            return {"valid": False, "reason": str(e), "details": {"errors": [str(e)]}}
        ...
        if args.full:
            ...
            variant_result = self._run_variant(rule_json)
        ...
        if args.elastic or args.full:
            ...
            elastic_result = self.siem_handler.query_logs(rule_json, logic_result, atomic_result)
    ...
    def _run_variant(self, rule_json):
        return self.variant_agent.evaluate_variants(rule_json)
